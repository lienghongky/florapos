...
export function Sidebar() {
  ...
  // ── Desktop sidebar (md+): collapsible fixed sidebar ──────────────────────
  const desktopSidebar = (
    <motion.div
      animate={{ width: isSidebarCollapsed ? 80 : 260 }}
      transition={{ type: 'spring', stiffness: 300, damping: 30 }}
      className="fixed inset-y-0 left-0 z-50 hidden md:flex h-full flex-col bg-white pb-4 text-foreground shadow-sm border-r border-border"
    >
      {/* Brand */}
  ...
  // ── Mobile drawer (< md): slides in from left as full overlay ─────────────
  const mobileSidebar = (
    <AnimatePresence>
      {isMobileSidebarOpen && (
        <motion.div
          ...
          exit={{ x: -280 }}
          transition={{ type: 'spring', stiffness: 300, damping: 30 }}
          className="fixed inset-y-0 left-0 z-50 flex md:hidden h-full w-[280px] flex-col bg-white pb-4 text-foreground shadow-2xl border-r border-border"
        >
          {/* Brand + close button */}
